    ...
    <div class="col-lg-6">
        <div class="rp-card mb-4">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6"><div class="text-muted small">Method</div><div>{{ str_replace('_', ' ', ucfirst($payment->payment_method instanceof \BackedEnum ? $payment->payment_method->value : $payment->payment_method)) }}</div></div>
        ...
        <div class="rp-card mb-4">
            ...
            <div class="row g-3">
                <div class="col-md-6"><div class="text-muted small">Booking ID</div><div><a href="{{ route('front_desk.reservations.show', $payment->booking) }}">{{ $payment->booking->short_number ?? $payment->booking->booking_number }}</a></div></div>
                <div class="col-md-6"><div class="text-muted small">Guest</div><div>{{ $payment->booking->guest_name ?? $payment->booking?->guest?->user?->name ?? '—' }}</div></div>
                ...
                <div class="col-md-6">
                    ...
                    <div><span class="text-muted">Total</span> ₱{{ number_format($payment->booking->total_amount, 2) }}</div>
                    <div><span class="text-muted">Paid</span> ₱{{ number_format($payment->booking->paid_amount, 2) }}</div>
                    <div><span class="text-muted">Due</span> ₱{{ number_format($payment->booking->remaining_balance, 2) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body pt-2">
                <div class="rp-receipt-card" id="rpPaymentReceiptPrint">
                    ...
                    <div class="rp-receipt-row"><span>Method</span><strong>{{ str_replace('_', ' ', ucfirst($payment->payment_method instanceof \BackedEnum ? $payment->payment_method->value : $payment->payment_method)) }}</strong></div>
                    ...
                    <div class="rp-receipt-row"><span>Status</span><strong>{{ str_replace('_', ' ', ucfirst($payment->status instanceof \BackedEnum ? $payment->status->value : $payment->status)) }}</strong></div>
                    @if($payment->verified_at)
...
<div class="modal fade" id="rpPaymentProofModal" tabindex="-1" aria-labelledby="rpPaymentProofModalLabel" aria-hidden="true"><div class="modal-dialog modal-dialog-centered modal-lg"><div class="modal-content">
    ...
    <div class="modal-body pt-3"><img src="{{ $payment->proof_url }}" alt="Payment proof screenshot" class="rp-payment-proof-modal-img"><div class="small text-muted mt-3 mb-0">Ref: {{ $payment->reference_number ?: '—' }} · ₱{{ number_format($payment->amount, 2) }} · {{ str_replace('_', ' ', ucfirst($payment->payment_method instanceof \BackedEnum ? $payment->payment_method->value : $payment->payment_method)) }}</div></div>
</div></div></div>
@endif
@endsection
