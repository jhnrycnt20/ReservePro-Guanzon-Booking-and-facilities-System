...
<head>
    ...
    <style>
        ...
        .receipt { max-width: 480px; margin: 2rem auto; background: #fff; border: 1px solid #ddd; padding: 2rem; }
        .receipt h1 { font-size: 1.4rem; margin-bottom: .25rem; }
        .meta { color: #666; font-size: .9rem; }
        .row-line { display: flex; justify-content: space-between; margin: .4rem 0; }
        @media print { body { background: #fff; } .no-print { display: none !important; } .receipt { border: none; margin: 0; max-width: none; } }
    </style>
</head>
<body>
    <div class="receipt">
        <div class="d-flex justify-content-between align-items-start mb-3">
            <div><h1>ReservePro</h1><div class="meta">Payment Receipt</div></div>
        ...
        <div class="row-line"><span>Method</span><strong>{{ str_replace('_', ' ', ucfirst($payment->payment_method instanceof \BackedEnum ? $payment->payment_method->value : $payment->payment_method)) }}</strong></div>
        ...
        <div class="row-line"><span>Status</span><strong>{{ str_replace('_', ' ', ucfirst($payment->status instanceof \BackedEnum ? $payment->status->value : $payment->status)) }}</strong></div>
        @if($payment->verified_at)<div class="row-line"><span>Verified at</span><strong>{{ $payment->verified_at->format('M d, Y g:i A') }}</strong></div>@endif
        <hr>
